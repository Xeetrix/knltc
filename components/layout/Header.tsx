...
function MainHeader({ pathname }: { pathname: string }) {
  ...
  return (<>
    <div className="bg-[#A8141A] py-2 text-xs text-white sm:text-sm"><div className="container-narrow flex flex-wrap items-center justify-between gap-2 text-white/85"><a href={siteConfig.phoneHref} className="inline-flex items-center gap-1.5 hover:text-white"><Phone className="h-3.5 w-3.5" />{siteConfig.phoneDisplay}</a><a href={siteConfig.whatsappHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-emerald-200 hover:text-white"><MessageCircle className="h-3.5 w-3.5" />{t.whatsapp}: {siteConfig.whatsappDisplay}</a></div></div>
    <header className={`sticky top-0 z-50 transition-all ${scrolled ? "border-b border-red-100 bg-white/90 shadow-md backdrop-blur-xl" : "bg-white"}`}>
      <div className="container-narrow flex min-h-[84px] items-center justify-between gap-4 py-3">
        <BrandLogo className="max-w-full" />
        <nav className="hidden flex-1 items-center justify-center gap-1 xl:flex">{navLinks.map((link) => { const Icon = link.icon; return <Link key={link.href} href={link.href} className={`group relative flex h-11 items-center gap-2 rounded-full px-4 text-sm font-medium transition ${isActive(link.href) ? "bg-[#D61F26] text-white" : "text-slate-700 hover:bg-red-50 hover:text-[#A8141A]"}`}><Icon className="h-4 w-4" />{link.label}</Link>; })}</nav>
        <div className="hidden items-center gap-2 xl:flex"><select className="rounded-full border border-red-200 bg-white px-3 py-2 text-xs" value={language} onChange={(e) => setLanguage(e.target.value as typeof language)}>{languageOptions.map((option) => <option key={option.value} value={option.value}>{`${option.short} · ${option.label}`}</option>)}</select><Button asChild className="h-11 rounded-full bg-gradient-to-r from-[#009966] to-[#00B67A] px-5 text-white shadow-[0_8px_20px_rgba(0,153,102,0.25)] transition-all hover:-translate-y-0.5 hover:shadow-[0_12px_24px_rgba(0,153,102,0.35)]"><Link href="/contact">{t.cta}</Link></Button></div>
        <button ref={menuButtonRef} className="rounded-full border border-red-200 p-2 xl:hidden" onClick={() => setMobileOpen((prev) => !prev)} aria-label={t.toggleMenu}>{mobileOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
      </div>
      {mobileOpen && <div className="border-t border-red-100 bg-white/95 p-3 backdrop-blur xl:hidden"><nav ref={menuPanelRef} className="space-y-2 rounded-2xl border border-red-100 bg-white p-3 shadow-xl"><select className="mb-2 w-full rounded-lg border border-red-200 p-2 text-sm" value={language} onChange={(e) => setLanguage(e.target.value as typeof language)}>{languageOptions.map((option) => <option key={option.value} value={option.value}>{`${option.short} · ${option.label}`}</option>)}</select>{navLinks.map((link) => { const Icon = link.icon; return <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm ${isActive(link.href) ? "bg-[#D61F26] text-white" : "text-slate-700 hover:bg-red-50"}`}><Icon className="h-4 w-4" />{link.label}</Link>; })}<Button asChild className="mt-2 w-full rounded-full bg-gradient-to-r from-[#009966] to-[#00B67A] hover:opacity-95"><Link href="/contact">{t.cta}</Link></Button></nav></div>}
    </header>
  </>);
}
